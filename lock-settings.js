export const LOCK_TIMEOUT_STORAGE_KEY = "circle-signal-lock-timeout-minutes";
...
export function readLockTimeout(storage = globalThis.localStorage) {
  try {
    return normalizeLockTimeout(storage?.getItem(LOCK_TIMEOUT_STORAGE_KEY));
  }
...
export function writeLockTimeout(minutes, storage = globalThis.localStorage) {
  ...
  try {
    storage?.setItem(LOCK_TIMEOUT_STORAGE_KEY, String(normalized));
  }
