...
test("generated setup QR codes decode to the exact original payload", () => {
  const setupCode = `CS2-${"A1b2C3d4E5f6G7h8J9k0".repeat(18)}`;
...
test("the scanner accepts only bounded CircleSignal setup payloads", () => {
  assert.equal(normalizeScannedSetupCode("  CS1-abc123  "), "");
  assert.equal(normalizeScannedSetupCode("  CS2-encrypted123  "), "CS2-encrypted123");
  assert.equal(normalizeScannedSetupCode("https://example.com"), "");
  assert.equal(normalizeScannedSetupCode(`CS2-${"a".repeat(20_000)}`), "");
  assert.equal(normalizeScannedSetupCode("CS3-unsupported"), "");
});
